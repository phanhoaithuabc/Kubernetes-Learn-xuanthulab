...
## ReplicaSet
Là một điều khiển Controller - nó đảm bảo ổn định các nhân bản (số lượng và tình trạng của POD, replica) khi đang chạy.
...
```bash
kubectl apply -f ReplicaSet/1.rs.yaml

# lấy các ReplicaSet thực hiện lệnh
kubectl get rs
...
# Để linh loạt và quy chuẩn, nên tạo ra HPA từ cấu hình file yaml
kubectl apply -f ReplicaSet/2.hpa.yaml
```
## Deployment 
...
```bash
# Apply deployment:
kubectl apply -f Deployment/1.myapp-deploy.yaml
# Deploy này sinh ra một ReplicasSet và quản lý nó

# kiểm tra deployment:
kubectl get deploy -o wide

# Edit deploy app:
kubectl edit deploy/deployapp deployment-name -n name-space
...
# Có thể cập nhật tài nguyên POD theo cách tương tự, ví dụ giới hạn CPU, Memory cho container với tên app-node
...
kubectl get hpa/deployapp name -o yaml > 2.hpa.yaml
```
...
Service (micro-service) là một đối tượng trừu tượng nó xác định ra một nhóm các POD và chính sách để truy cập đến nhóm POD đó. Nhóm POD mà Service xác định thường dùng kỹ thuật Selector (chọn các POD thuộc về Service theo label của POD). Cũng có thể hiểu Service là một dịch vụ mạng, tạo cơ chế cân bằng tải (load balancing) truy cập đến các điểm cuối (thường là các Pod) mà Service đó phục vụ.
```bash
# apply service 
kubectl apply -f "Service - svc/1.svc1.yaml"
...
kubectl apply -f "Service - svc/2.endpoint.yaml"
...
kubectl apply -f "Service - svc/3.pods.yaml"
...
kubectl apply -f "Service - svc/4.svc2.yaml"
...
kubectl appy -f "Service - svc/5.svc3.yaml"
